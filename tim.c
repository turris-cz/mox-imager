...
static void emit_otp_read_row(u32 sm_row, u32 sm_store0, u32 sm_store1,
			      u32 sm_sfb)
{
...
static void emit_print_sm(u32 sm, u32 *label)
{
...
static void emit_otp_read(void)
{
	u32 label = 4;
	...
	emit_print("U ");
	emit(LABEL, 3);
	emit_print_sm(2, &label);
	emit_putc(' ');
	emit_print_sm(1, &label);
	emit_print("\r\n");
	emit(ADD_SM_VAL, 0, 1);
	emit(TEST_SM_AND_BRANCH, 0, 0xffffffff, 44, OP_NE, 1);
}

static void emit_otp_write(void)
{
